...
TEST_BEGIN("BFS")
{
	...
	convert << rank;
	IsTrue("Directed Rank B", rank == base_rank + 1, convert.str());
	c->getDiscoveryInformation(color, dt, ft, rank);
	ostringstream convert2;
	convert2 << rank;
	IsTrue("Directed Rank C", rank == base_rank + 1, convert2.str());
	d->getDiscoveryInformation(color, dt, ft, rank);
	ostringstream convert3;
	convert3 << rank;
	IsTrue("Directed Rank D", rank == base_rank + 2, convert3.str());
	e->getDiscoveryInformation(color, dt, ft, rank);
	ostringstream convert4;
	convert4 << rank;
	IsTrue("Directed Rank E", rank == base_rank + 1, convert4.str());
	f->getDiscoveryInformation(color, dt, ft, rank);
	ostringstream convert5;
	convert5 << rank;
	IsTrue("Directed Rank F", rank == base_rank + 3, convert5.str());
	g->getDiscoveryInformation(color, dt, ft, rank);
	ostringstream convert6;
	convert6 << rank;
	IsTrue("Directed Rank G", rank == base_rank + 2, convert6.str());
}TEST_END
...
TEST_BEGIN("BFSUndirected")
{
	...
	ostringstream convert;
	convert << rank;
	IsTrue("Undirected Rank C", rank == base_rank + 1, convert.str());
	d->getDiscoveryInformation(color, dt, ft, rank);
	ostringstream convert2;
	convert2 << rank;
	IsTrue("Undirected Rank D", rank == base_rank + 2, convert2.str());
	e->getDiscoveryInformation(color, dt, ft, rank);
	ostringstream convert3;
	convert3 << rank;
	IsTrue("Undirected Rank E", rank == base_rank + 1, convert3.str());
	f->getDiscoveryInformation(color, dt, ft, rank);
	ostringstream convert4;
	convert4 << rank;
	IsTrue("Undirected Rank F", rank == base_rank + 2, convert4.str());
	g->getDiscoveryInformation(color, dt, ft, rank);
	ostringstream convert5;
	convert5 << rank;
	IsTrue("Undirected Rank G", rank == base_rank + 2, convert5.str());
}TEST_END
SUITE_END
